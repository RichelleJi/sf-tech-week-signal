...
function runClassification(){
 ...
 panel.classList.remove('complete');panel.classList.add('running');state.textContent='RUNNING';button.disabled=true;updateLiveSignalMix(0);let step=0;
 ...
 const timer=setInterval(()=>{step++;const pct=Math.min(step*8,100),processed=Math.min(Math.round(pct/100*40),40),stageIndex=Math.min(Math.floor(pct/27),3),tokensIn=Math.round(78700*pct/100),tokensOut=Math.round(35300*pct/100);bar.style.width=`${pct}%`;detail.textContent=`${pct}% processed`;processedCount.textContent=`${processed} / 40`;inputTokens.textContent=tokensIn.toLocaleString();outputTokens.textContent=tokensOut.toLocaleString();totalTokens.textContent=(tokensIn+tokensOut).toLocaleString();updateLiveSignalMix(processed);status.textContent=stages[stageIndex];activity.textContent=stageIndex===2&&processed?`Evaluating “${events[Math.min(processed-1,39)].name}” against the weighted rubric.`:activityStages[stageIndex];if(pct===100){clearInterval(timer);setTimeout(()=>{panel.classList.remove('running');panel.classList.add('complete');state.textContent='COMPLETE';status.textContent='Classification complete';detail.textContent='100% processed';activity.textContent='40 events scored, ranked, and ready for review.';button.disabled=false;showToast('40 events classified · results updated')},450)}},120);
}
